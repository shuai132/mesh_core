...
struct Impl {
  ...
  static void broadcast(std::string data) {
    data.append((char*)&self_addr, sizeof(self_addr));
...
static void start_recv(Impl& impl) {
  ...
  s_socket.async_receive_from(asio::buffer(recv_buffer), sender_endpoint, [&](const asio::error_code& ec, std::size_t bytes_received) {
    mesh_core::addr_t recv_from = *(recv_buffer.data() + bytes_received - 1);
    if (!ec) {
      std::string payload = std::string(recv_buffer.data(), bytes_received - 1);
      MESH_CORE_LOGD("UDP recv from: 0x%02X, size: %zu", recv_from, payload.size());
      ...
      // ignore msg which addr diff > 2, for test routing
      if (std::abs(recv_from - self_addr) > 2) {
